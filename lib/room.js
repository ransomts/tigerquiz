...
export class Room {
   ...
   * @param {any} quiz
   * @param {any} roster
   * @param {string | null} owner eppn of the instructor running it, if signed in
   */
  constructor(io, pin, hostId, quiz, roster, owner = null) {
    this.io = io;
    this.owner = owner;
    this.id = randomUUID();
    this.pin = pin;
    ...
    store.createGame({ id: this.id, pin, quizId: quiz.id, title: quiz.title, rosterId: roster?.id, startedAt: Date.now(), owner });
  }
  ...
  restart() {
    ...
    store.createGame({
      ...
      rosterId: this.roster?.id, startedAt: Date.now(), owner: this.owner,
    });
