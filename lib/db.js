...
export function open(dir) {
  ...
  db.exec(`
    ...
    -- listStudents aggregates correctness across every answer ever recorded;
    -- this covers that GROUP BY so it reads the index instead of the table
    CREATE INDEX IF NOT EXISTS answers_by_player ON answers(game_id, player, correct);
  `);
  migrate();
  sweepUnfinished();
  return db;
