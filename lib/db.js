...
// ---------- reports ----------
/**
 * Finished games, newest first. With an eppn the list narrows to that
 * instructor's own games plus the unowned ones; without, it is everything,
 * which is what a deployment with sign-in switched off wants.
 */
export function listGames(limit = 100, eppn = null) {
  ...
  return all(
    `SELECT ${cols} FROM games
     WHERE ended_at IS NOT NULL AND (owner = ? OR owner IS NULL)
     ORDER BY started_at DESC LIMIT ?`, eppn, limit
  );
}
 ...
 */
export function listStudents(eppn = null) {
  const scope = eppn ? `AND (g.owner = ? OR g.owner IS NULL)` : ``;
  const args = eppn ? [eppn] : [];
  const rows = all(`
    ...
    WHERE g.ended_at IS NOT NULL ${scope}
