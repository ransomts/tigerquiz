...
import { requireInstructor, scopeFor, canRead, canWrite, canCreate, forbidden } from "./auth.js";
...
export function apiRouter() {
  ...
  api.get("/reports", (req, res) => res.json(store.listGames(100, scopeFor(req))));

  api.get("/students", (req, res) => res.json(store.listStudents(scopeFor(req))));
  ...
  api.get("/reports/:id", async (req, res) => {
    ...
    if (!canRead(req.user, report.game.owner)) return res.status(404).json({ error: "No such game" });
  ...
  api.post("/reports/:id/claim", (req, res) => {
    ...
    if (!canRead(req.user, owner)) return res.status(404).json({ error: "No such game" });
  ...
  api.delete("/reports/:id", (req, res) => {
    ...
    if (owner === undefined) return res.status(404).json({ error: "No such game" });
    if (!canRead(req.user, owner)) return res.status(404).json({ error: "No such game" });
    if (!canWrite(req.user, owner)) return res.status(403).json(forbidden(owner));
