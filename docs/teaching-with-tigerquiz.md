...
[host screen](https://brgr.cecas.clemson.edu/quiz/host), which signs you in with
your department account. The other two teacher pages this walkthrough uses are
the [editor](https://brgr.cecas.clemson.edu/quiz/edit) and the
[reports](https://brgr.cecas.clemson.edu/quiz/reports). Everything you make is
yours: your quizzes, your class lists and your reports, and nobody else's.
...
**Check the quiz.** `bin/check` reads every quiz and class list and reports
...
Open the host screen. Pick one of your quizzes, and optionally a class list.
...
![The results screen, showing one vote for the right answer, two for the same wrong one, and the explanation](images/06-results.png)
...
**The distribution** shows what people actually chose. Two people here picked
Venus and only one picked Mars. A low correct rate tells you a question was hard;
the wrong answer people landed on tells you what they believe instead, which is
the more useful fact — and the report later names it for you.
...
revealed together: Alan, Grace and Charlie are all level above, so all three are
second and there is no third at all. Nobody is quietly ranked above somebody they
drew with, which is what would happen if places were handed out by position in the
list — and the order in that list is only the order people happened to join in.
