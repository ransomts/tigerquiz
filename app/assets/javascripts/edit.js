...
// back to the server through the API, so no JSON has to be typed by hand.
...
async function showList() {
  ...
  for (const q of quizzes) {
    ...
    main.append(el("div", "card-title", q.title), el("div", "card-meta", q.error ? q.error : `${q.count} questions · ${q.id}`));
  ...
  for (const r of rosters) {
    ...
    main.append(el("div", "card-title", r.title), el("div", "card-meta", `${r.count} students · ${r.id}`));
    const del = el("button", null, "Delete");
    del.onclick = async () => {
      if (!confirm(`Delete the class list "${r.title}"?`)) return;
      await api(`/api/roster/${encodeURIComponent(r.id)}`, { method: "DELETE" });
      showList();
    };
...
async function removeQuiz(id, title) {
  ...
  await api(`/api/quiz/${encodeURIComponent(id)}`, { method: "DELETE" });
  showList();
}
...
$("saveBtn").onclick = async () => {
  ...
  if (!id) return alert("Give the quiz a title or a short name");
  $("fId").value = id;
...
$("rosterSaveBtn").onclick = async () => {
  ...
  if (!id) return alert("Give the class a name or a short name");
